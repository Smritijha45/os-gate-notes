# 📘 Memory Management – Complete GATE Notes (Operating Systems)

---

# 📌 1. What is Memory Management?

**Memory Management** is the functionality of the OS that:

✔ Allocates memory to processes  
✔ Keeps track of memory usage  
✔ Protects process memory  
✔ Ensures efficient utilization  

---

# 📌 2. Address Binding

Mapping logical address → physical address.

---

## 🔹 Types of Address Binding

1️⃣ Compile-Time  
2️⃣ Load-Time  
3️⃣ Execution-Time  

---

### 🔹 Compile-Time Binding
- Physical address known at compile time
- Absolute code generated

---

### 🔹 Load-Time Binding
- Address decided when program loaded
- Relocatable code

---

### 🔹 Execution-Time Binding
- Address mapped during execution
- Requires hardware support (MMU)

---

# 📌 3. Logical vs Physical Address

| Feature | Logical Address | Physical Address |
|----------|----------------|------------------|
| Generated By | CPU | Memory Unit |
| Also Called | Virtual Address | Real Address |
| Visible To | User | Hardware |

---

## 🔹 MMU (Memory Management Unit)

Hardware that translates logical address → physical address.

---

# 📌 4. Swapping

Process moved from main memory → disk  
Later brought back.

Used when:
- Memory is full
- Increase degree of multiprogramming

---

# 📌 5. Contiguous Memory Allocation

Process occupies contiguous block of memory.

---

## 🔹 Types:

1️⃣ Single Partition  
2️⃣ Fixed Partition  
3️⃣ Variable Partition  

---

# 📌 6. Fragmentation

---

## 🔹 Internal Fragmentation

Unused memory inside allocated block.

Occurs in:
- Fixed partition
- Paging

---

## 🔹 External Fragmentation

Free memory exists but not contiguous.

Occurs in:
- Variable partition
- Segmentation

---

# 📌 7. Solutions to Fragmentation

✔ Compaction  
✔ Paging  
✔ Segmentation  

---

# 📌 8. Paging (Very Important for GATE)

Memory divided into:

- Pages (Logical)
- Frames (Physical)

Page size = Frame size

---

## 🔹 Address Format in Paging

```
Logical Address = Page Number + Offset
```

---

## 🔹 Page Table

Stores mapping:
```
Page Number → Frame Number
```

---

## 🔹 Page Table Entry Contains

- Frame number
- Valid bit
- Protection bit
- Dirty bit
- Reference bit

---

# 📌 9. Paging Example

If:
- Page size = 1KB
- Logical address = 2050

Page number = 2050 / 1024  
Offset = 2050 % 1024

---

# 📌 10. Multi-Level Paging

Used when page table too large.

Instead of single page table:
- Divide into multiple levels

Reduces memory usage.

---

# 📌 11. TLB (Translation Lookaside Buffer)

Small, fast memory cache.

Stores recent page table entries.

---

## 🔹 TLB Hit

Page found in TLB → Faster access

## 🔹 TLB Miss

Search page table → Slower

---

## 🔹 Effective Memory Access Time (EAT)

```
EAT = h × (TLB time + Memory time) +
      (1 - h) × (TLB time + 2 × Memory time)
```

Where:
h = TLB hit ratio

---

# 📌 12. Segmentation

Memory divided into logical segments:

- Code
- Data
- Stack

---

## 🔹 Logical Address in Segmentation

```
< Segment Number, Offset >
```

---

## 🔹 Segment Table Contains

- Base address
- Limit

---

## 🔹 Comparison: Paging vs Segmentation

| Feature | Paging | Segmentation |
|----------|--------|--------------|
| Division | Fixed size | Variable size |
| Fragmentation | Internal | External |
| User View | No | Yes |

---

# 📌 13. Virtual Memory

Allows execution of processes not fully in memory.

Uses:
- Demand Paging

---

# 📌 14. Demand Paging

Pages loaded only when required.

---

## 🔹 Page Fault

Occurs when page not in memory.

Steps:
1. Trap to OS
2. Load page from disk
3. Update page table
4. Restart instruction

---

# 📌 15. Page Replacement Algorithms (Very Important)

When memory is full:

---

## 🔹 1. FIFO

Replace oldest page.

❌ Belady’s anomaly possible.

---

## 🔹 2. LRU (Least Recently Used)

Replace least recently used page.

✔ Better than FIFO  
❌ Hard to implement perfectly  

---

## 🔹 3. Optimal

Replace page not used for longest future time.

✔ Minimum page faults  
❌ Not implementable (theoretical)

---

## 🔹 4. LFU (Least Frequently Used)

Replace page with lowest reference count.

---

# 📌 16. Belady’s Anomaly

Increasing frames increases page faults.

Occurs in:
✔ FIFO  
❌ Not in LRU or Optimal  

---

# 📌 17. Thrashing

When:
- Too many page faults
- CPU utilization drops

System spends more time swapping than executing.

---

# 📌 18. Working Set Model

Keeps track of pages used recently.

Helps prevent thrashing.

---

# 📌 19. Important GATE Points

✔ Logical vs Physical address  
✔ TLB EAT formula  
✔ Paging calculations  
✔ Page replacement numericals  
✔ Belady’s anomaly  
✔ Segmentation vs Paging  
✔ Internal vs External fragmentation  

---




